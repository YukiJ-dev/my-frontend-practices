...
(function() {
    ...
    (function() {
        ...
        (function() {
            ...
            let mobileStyledElems = [innerNav, mainNav, body];
            let isOpend;

            // open mobile menu

            menuBtn.addEventListener("click", event => {
                isOpend = !mainNav.classList.contains("opend-menu");
            ...
            matchQuery_1024px.addEventListener("change", event => {
                if (isOpend && event.matches) {
                    mobileStyledElems.forEach(htmlEl => htmlEl.classList.remove("opend-menu"));
                    logo.dataset.menuIsOpend = false;
                } else if (isOpend && !event.matches) {
                    mobileStyledElems.forEach(htmlEl => htmlEl.classList.add("opend-menu"));
                    logo.dataset.menuIsOpend = true;
                }
            });
        }());
        ...
        (function() {
            ...
            // for getting the y coordinate for top postion of the popup
            let salesWrapper = document.getElementById("sales-wrapper");

            let toggleBtn = document.getElementById("submenu-btn");
            let subMenuWrapper = salesWrapper.querySelector(".sales-submenu-wrapper");

            function calculateXInfelicityOfPopup() {
                let innerSvgStyles = window.getComputedStyle(subMenuWrapper.firstElementChild);
                let dropDownStyles = window.getComputedStyle(toggleBtn, "::after");

                let xInfelicity = ((parseFloat(dropDownStyles.width) + parseFloat(innerSvgStyles.width)) / 2) + parseFloat(innerSvgStyles.left);

                return xInfelicity;
            }

            // handle clicked event for open popup menu
            toggleBtn.addEventListener("click", event => {
                if (subMenuWrapper.classList.toggle("opend-submenu")) {
                    let xInfelicity = calculateXInfelicityOfPopup();

                    let x = toggleBtn.getBoundingClientRect().right - salesWrapper.getBoundingClientRect().x;
                    let y = salesWrapper.offsetHeight + window.pageYOffset;
                ...
                toggleBtn.classList.toggle("opend-submenu");
                event.stopPropagation();
            });

            let styledElements = [toggleBtn, subMenuWrapper];

            // hide when user click at another place
            document.addEventListener("click", event => {
                if (toggleBtn.classList.contains("opend-submenu"))
                    styledElements.forEach(htmlEl => htmlEl.classList.remove("opend-submenu"));
            });

            const resizeObserver = new ResizeObserver(entries => {
                if (subMenuWrapper.classList.contains("opend-submenu")) {
                    styledElements.forEach(htmlEl => htmlEl.classList.remove("opend-submenu"));
                }
            });

            resizeObserver.observe(toggleBtn);
        })();
    })();
    ...
    (function() {
        ...
        innerCards.forEach(card => {
            card.addEventListener("click", event => {
                // if blog card isn't choosed
                if (card.isChoosed != "true") {
    ...
    (function() {
        ...
        avatars.forEach(avatar => {
            // when user hover the avatar
            avatar.addEventListener("mouseenter", event => {
                lastChoosedAvatar = avatar;

                // ResizeObserver is triggered
                avatarsWrapper.append(dialogWindow);
                paintDialogWindow(avatar);
                dialogWindow.classList.add("active");
                // get last width after adding dialog window to page
                lastWidthOfWindow = dialogWindow.getBoundingClientRect().width;
            });

            // when user go out from avatar to next
